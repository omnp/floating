...
static double blend (double t, double x, double y) { return (1.0 - t) * x + t * y; }
...
add_top_layer (FloatingDrawing *drawing, int width, int height)
{
  ...
  if (current != NULL) {
    current->next = floating_layer_new (current->image->width, current->image->height);
...
void update(FloatingDrawing *drawing,
	    rect invalid_area,
	    int image_width,
	    int image_height,
	    uint32_t *image,
	    ...
	    uint8_t background)
{
  ...
  if (invalid_area.x < image_width && invalid_area.y < image_height &&
	...
	for (j = 0; j < width; ++j) {
	  ...
	  unsigned int current_index = (y + i) * current->image->width + x + j;
	  if (x + j >= 0 && x + j < image_width && y + i < image_height && y + i >= 0) {
	    color final_color = scratch->data[scratch_index];
	    color current_color = current->image->data[current_index];
	    if (current != drawing->bottom) {
	      if (current->alpha > 0 && current_color.alpha > 0) {
		const float alpha = final_color.alpha;
		const float current_alpha = current_color.alpha;
		const float inv_alpha =  1 / (alpha * (1 - current_alpha) + current->alpha * current_alpha);
		color_multiply_single_struct (final_color.alpha, final_color.vector, &final_color);
		color_multiply_single_struct (current->alpha, current_color.vector, &current_color);
		color_blend_single_struct (current_alpha, final_color.vector, current_color.vector, &final_color);
		color_multiply_single_struct (inv_alpha, final_color.vector, &final_color);
		final_color.alpha = fmin (1.0, alpha + current->alpha * current_alpha);
	      }
	    }
	    else {
	      final_color = current_color;
	      final_color.alpha = current->alpha * final_color.alpha;
	    }
	...
	if (x + j >= 0 && x + j < image_width && y + i < image_height && y + i >= 0) {
	  ...
	  surface_data[surface_index + 0] = scratch->data[scratch_index].red * 255;
	  surface_data[surface_index + 1] = scratch->data[scratch_index].green * 255;
	  surface_data[surface_index + 2] = scratch->data[scratch_index].blue * 255;
	  surface_data[surface_index + 3] = scratch->data[scratch_index].alpha * 255;
	  tmp_data[4*scratch_index + 0] = blend(f, background, surface_data[surface_index + 2]);
	  tmp_data[4*scratch_index + 1] = blend(f, background, surface_data[surface_index + 1]);
	  tmp_data[4*scratch_index + 2] = blend(f, background, surface_data[surface_index + 0]);
	  tmp_data[4*scratch_index + 3] = blend(f, background, surface_data[surface_index + 3]);
	}
      }
    }
...
main(int argc, char **args)
{
  ...
  const char vendor_name[] = "Wacom", stylus[] = "Pen stylus", finger[] = "Finger";
  uint8_t graphics_tablet_stylus_device_id = 0; /* Will find what the correct id is later. */
  ...
  memset((void *) image, BACKGROUND, sizeof(uint32_t) * image_width * image_height);
  ...
  xcb_put_image(connection,
		...
		draw,
	        image_width, image_height,
		0, 0,
		0, 24, (4 * image_width * image_height), (void *) image);
  xcb_flush(connection);
  ...
  devices_cookie = xcb_input_xi_query_device(connection, XCB_INPUT_DEVICE_ALL);
  devices_reply = xcb_input_xi_query_device_reply(connection, devices_cookie, NULL);
  xcb_input_xi_device_info_iterator_t devices_iterator = \
    xcb_input_xi_query_device_infos_iterator(devices_reply);
  for (; devices_iterator.rem; xcb_input_xi_device_info_next(&devices_iterator)) {
    xcb_input_xi_device_info_t *info = devices_iterator.data;
    char *name = xcb_input_xi_device_info_name(info);
    printf("%d %d %s %d %d\n", info->deviceid, info->type, name, info->attachment, info->num_classes);
	...
	xcb_input_xi_device_info_classes_iterator(info);
      for (; class_iterator.rem; xcb_input_device_class_next(&class_iterator)) {
	  ...
	  if (class_data) {
	    {
	      ...
	      min = (((double) class_data->min.integral) + ((double) class_data->min.frac) / divider);
	      max = (((double) class_data->max.integral) + ((double) class_data->max.frac) / divider);
	      range = max - min;
	      printf("%d res = %d min = %f max = %f range = %f\n",
		     class_data->number, res, min, max, range);
	    }
	    if (class_data->number + 1 == graphics_tablet_stylus_pressure_axis_number) {
	      graphics_tablet_stylus_pressure_axis_resolution = class_data->resolution;
	      graphics_tablet_stylus_pressure_axis_min = (((double) class_data->min.integral) + ((double) class_data->min.frac) / divider);
	      graphics_tablet_stylus_pressure_axis_max = (((double) class_data->max.integral) + ((double) class_data->max.frac) / divider);
	      printf("Pressure axis number %d with min = %f, max = %f, and resolution = %d, range = %f\n",
		     ...
		     graphics_tablet_stylus_pressure_axis_resolution, graphics_tablet_stylus_pressure_axis_max - graphics_tablet_stylus_pressure_axis_min);
	    }
	    if (class_data->number + 1 == graphics_tablet_stylus_x_axis_number) {
	      graphics_tablet_stylus_x_axis_resolution = class_data->resolution;
	      graphics_tablet_stylus_x_axis_min = (((double) class_data->min.integral) + ((double) class_data->min.frac) / divider);
	      graphics_tablet_stylus_x_axis_max = (((double) class_data->max.integral) + ((double) class_data->max.frac) / divider);
	      printf("X axis number %d with min = %f, max = %f, and resolution = %d, range = %f\n",
		     graphics_tablet_stylus_x_axis_number,
		     graphics_tablet_stylus_x_axis_min,
		     graphics_tablet_stylus_x_axis_max,
		     graphics_tablet_stylus_x_axis_resolution, graphics_tablet_stylus_x_axis_max - graphics_tablet_stylus_x_axis_min);
	    }
	    if (class_data->number + 1 == graphics_tablet_stylus_y_axis_number) {
	      graphics_tablet_stylus_y_axis_resolution = class_data->resolution;
	      graphics_tablet_stylus_y_axis_min = (((double) class_data->min.integral) + ((double) class_data->min.frac) / divider);
	      graphics_tablet_stylus_y_axis_max = (((double) class_data->max.integral) + ((double) class_data->max.frac) / divider);
	      printf("Y axis number %d with min = %f, max = %f, and resolution = %d, range = %f\n",
		     graphics_tablet_stylus_y_axis_number,
		     graphics_tablet_stylus_y_axis_min,
		     graphics_tablet_stylus_y_axis_max,
		     graphics_tablet_stylus_y_axis_resolution, graphics_tablet_stylus_y_axis_max - graphics_tablet_stylus_y_axis_min);
	    }
	  }
	}
      }
  ...
  if (graphics_tablet_stylus_device_id) {
    ...
    const uint32_t mask[] = { XCB_EVENT_MASK_POINTER_MOTION |
			      XCB_EVENT_MASK_BUTTON_PRESS |
			      XCB_EVENT_MASK_BUTTON_RELEASE };
    const uint32_t mods[] = { XCB_INPUT_MODIFIER_MASK_ANY };
    cookie = xcb_input_xi_passive_grab_device(connection,
					      XCB_CURRENT_TIME,
					      screen->root,
					      XCB_CURSOR_NONE,
					      0,
					      ...
					      1,
					      1,
					      XCB_INPUT_GRAB_TYPE_FOCUS_IN,
					      XCB_INPUT_GRAB_MODE_22_ASYNC,
					      XCB_INPUT_GRAB_MODE_22_ASYNC,
					      XCB_INPUT_GRAB_OWNER_NO_OWNER,
					      mask,
					      mods);
    reply = xcb_input_xi_passive_grab_device_reply(connection, cookie, NULL);
    free(reply);
    struct {
      xcb_input_event_mask_t iem;
      xcb_input_xi_event_mask_t xiem;
  ...
  while ((event = xcb_wait_for_event(connection))) {
	...
	xcb_input_raw_button_press_event_t * rbt_event = (void *) event;
	if (rbt_event->deviceid > 3 &&
	    rbt_event->deviceid == graphics_tablet_stylus_device_id) {
	  if (rbt_event->detail == 1) {
	    drawing->is_drawing = 1;
	    pressure = 1.0;
	    int axis_len = xcb_input_raw_button_press_axisvalues_length(rbt_event);
	    if (axis_len) {
	      xcb_input_fp3232_t *axisvalues = xcb_input_raw_button_press_axisvalues_raw(rbt_event);
	      int i = 0;
	      for (; i < axis_len; ++i) {
		xcb_input_fp3232_t value = axisvalues[i];
		double dbl_value = ((double) value.integral + (double) value.frac / divider);
		if (i == graphics_tablet_stylus_x_axis_number - 1) {
		  long pos_x = root_width * dbl_value / (graphics_tablet_stylus_x_axis_max - graphics_tablet_stylus_x_axis_min);
		  drawing->x = pos_x - win_pos_x;
		}
		if (i == graphics_tablet_stylus_y_axis_number - 1) {
		  long pos_y = root_height * dbl_value / (graphics_tablet_stylus_y_axis_max - graphics_tablet_stylus_y_axis_min);
		  drawing->y = pos_y - win_pos_y;
		}
		if (i == graphics_tablet_stylus_pressure_axis_number - 1) {
		  pressure = pressure * (float) (dbl_value / graphics_tablet_stylus_pressure_axis_max);
		}
	      }
	    }
	  }
	...
	xcb_input_raw_button_release_event_t * rbt_event = (void *) event;
	if (rbt_event->deviceid > 3 &&
	    rbt_event->deviceid == graphics_tablet_stylus_device_id) {
	  if (rbt_event->detail == 1) {
	...
	if (rmt_event->deviceid > 3 &&
	    rmt_event->deviceid == graphics_tablet_stylus_device_id) {
	  int axis_len = xcb_input_raw_button_press_axisvalues_length(rmt_event);
	  if (axis_len) {
	    xcb_input_fp3232_t *axisvalues = xcb_input_raw_button_press_axisvalues_raw(rmt_event);
	    int i = 0;
	    for (; i < axis_len; ++i) {
	      xcb_input_fp3232_t value = axisvalues[i];
	      double dbl_value = ((double) value.integral + (double) value.frac / divider);
		...
		drawing->y = pos_y - win_pos_y;
	      ...
	      if (i == graphics_tablet_stylus_pressure_axis_number - 1) {
		pressure = (float) (dbl_value / graphics_tablet_stylus_pressure_axis_max);
	      }
	    }
	  }
	}
	...
	if (bt_event->deviceid != graphics_tablet_stylus_device_id) {
	  uint32_t *button_mask = xcb_input_button_press_button_mask(bt_event);
	  int button_len = xcb_input_button_press_button_mask_length(bt_event);
	  if (bt_event->deviceid > 3 && button_len && bt_event->detail == 1 && !button_mask[0]) {
	    drawing->is_drawing = 1;
	    pressure = 1.0;
	    win_pos_x = (bt_event->root_x >> 16) - (bt_event->event_x >> 16);
	    win_pos_y = (bt_event->root_y >> 16) - (bt_event->event_y >> 16);
	...
	if (mt_event->deviceid > 3 &&
	    mt_event->deviceid != graphics_tablet_stylus_device_id) {
	...
	break;
      ...
      if (!drawing->is_drawing || drawing->current == NULL || drawing->current->image == NULL) {
	break;
      }
      if (pressure <= 0.0) {
	break;
	...
	if (brush->is_drawing && brush_density > 0 && brush_radius > 0 && brush_hardness > 0) {
	  double t;
	  for (t = 0.0; t < 1.0 + brush_density; t += brush_density) {
	    const double x = t * drawing->x + (1 - t) * prev_x;
	    const double y = t * drawing->y + (1 - t) * prev_y;
	    const int xi = x, yi = y;
	    const double brush_radius_sq = brush_radius * brush_radius;
	    unsigned int total_pixels = 0;
	    color total_color = {{0, 0, 0, 0}};
	    /* Draw circular brush mark. */
	    const int brush_bounding_size = 2 * ceil(brush_radius) + 1;
	    int i;
            #pragma omp parallel for
	    for (i = xi - ceil(brush_radius); i <= xi + brush_bounding_size; ++i) {
	      int j;
              #pragma omp parallel for
	      for (j = yi - ceil(brush_radius); j <= yi + brush_bounding_size; ++j) {
		double blend_factor = 0.0;
		double alpha = 1.0;
		double distance_sq = (i - x) * (i - x) + (j - y) * (j - y);
		if (i >= 0 && j >= 0 && i < width && j < height && distance_sq <= brush_radius_sq) {
		  unsigned int index = (j * width + i);
		  color final_color = pix[index];
		  color brush_color;
		  color_blend_single_struct (brush->medium_color.alpha, brush->color.vector, brush->medium_color.vector, &brush_color);
		  if (distance_sq / brush_radius_sq >= brush_hardness) {
		    alpha = brush_hardness * brush_hardness * brush_radius_sq / distance_sq;
		  }
		  alpha *= brush_alpha;
		  if (brush->is_erasing) {
		    if (final_color.alpha > 0) {
		      blend_factor = alpha * brush_color.alpha;
		      if (final_color.alpha > 0) {
			final_color.alpha = blend (blend_factor, final_color.alpha, 0);
		      }
		    }
		  }
		  else {
		    if (final_color.alpha > 0) {
		      blend_factor = alpha * brush_color.alpha;
		      color_blend_single_struct (blend_factor, final_color.vector, brush_color.vector, &final_color);
		    }
		    else {
		      final_color = brush_color;
		      final_color.alpha = alpha * final_color.alpha;
		    }
                    #pragma omp critical
		    {
		      color_add_struct (total_color.vector, final_color.vector, &total_color);
		      total_pixels += 1;
		    }
		  }
		  if (!brush->is_picking) {
		...
		color_blend_single_struct (brush_smudge, brush->color.vector, total_color.vector, &brush->color);
	      }
	      if (brush->is_picking) {
	    ...
	    invalid_area.width = max(invalid_area.width, invalid_area_width);
	    invalid_area.height = max(invalid_area.height, invalid_area_height);
	  }
	}
	brush = brush->next;
      }
    ...
    case XCB_KEY_PRESS: {
      ...
      case 31: { /*key: i; increase brush size*/
	if (key_event->state & XCB_MOD_MASK_SHIFT) { /*key: shift-i; decrease brush size*/
	  Brush *brush = drawing->active_brushes;
	  while (brush != NULL) {
	    brush->radius -= 1;
	    if (brush->radius < 0) {
	...
	break;
      ...
      case 32: { /*key: o; increase brush alpha*/
	if (key_event->state & XCB_MOD_MASK_SHIFT) { /*key: shift-o; decrease brush alpha*/	
	  Brush *brush = drawing->active_brushes;
	  while (brush != NULL) {
	...
	break;
      ...
      case 39: { /*key: s; maybe save image file*/
	if (key_event->state & XCB_MOD_MASK_SHIFT) { /*shift-s saves image data to file*/
	  if (image_file_name) {
	    ...
	    if (tif) {
	      ...
	      TIFFSetField(tif, TIFFTAG_TILEWIDTH, image_width);
	      TIFFSetField(tif, TIFFTAG_TILELENGTH, image_height);
	      TIFFWriteTile(tif, image, 0, 0, 0, 0);
	      TIFFClose(tif);
	      printf("Saved image to file %s\n", image_file_name);
	    }
	  }
	}
	else { /*key: s; toggle smudge on / off*/
	  Brush *brush = drawing->active_brushes;
	  while (brush != NULL) {
	...
	break;
      ...
      case 56: { /*key: b; toggle paint on / off*/
	...
	break;
      ...
      case 26: { /*key: e; toggle erase on / off*/
	Brush *brush = drawing->active_brushes;
	while (brush != NULL) {
	...
	break;
      ...
      case 33: { /*key: p; toggle pick on / off*/
	Brush *brush = drawing->active_brushes;
	while (brush != NULL) {
	...
	break;
      ...
      case 46: { /*key: l; layer management*/
	if (key_event->state & XCB_MOD_MASK_SHIFT) { /*shift-l deletes topmost layer*/
	  del_top_layer(drawing);
	  if (drawing->current == NULL) {
	    memset((void *) image, 0x0, sizeof(uint32_t) * image_width * image_height);
	  }
	...
	break;
      ...
      case 54: { /*key: c; next color*/
	if (colors_index >= 0) {
	  if (key_event->state & XCB_MOD_MASK_SHIFT) { /*shift-c previous color*/
	    colors_index -= 1;
	    if (colors_index < 0) {
